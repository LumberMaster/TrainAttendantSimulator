// Прозрачный материал с ORM-упаковкой. Непрозрачность из альфы Base Color.
//
// Блендинг premultiplied (One OneMinusSrcAlpha): альфой гасится только
// альбедо, а блики и отражения идут на полную силу. При обычном
// SrcAlpha стекло с непрозрачностью 0.2 теряет 80% отражений и
// превращается в мутную плёнку.
//
// Собран под WebGL: один проход и минимум ключевых слов. Выброшены
// лайтмапы (прозрачная геометрия в них не попадает), SSAO (не работает
// на прозрачном), тени от дополнительных источников, декали и блендинг
// reflection probe - каждая такая строка удваивает число вариантов,
// которые браузер компилирует при первом запуске.
Shader "Custom/ORMOpacity_URP"
{
    Properties
    {
        [MainTexture] _BaseMap ("Base Color (A = Opacity)", 2D) = "white" {}
        ...
        _SmoothnessMin ("Smoothness Floor", Range(0,1)) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Альфа из текстуры почти никогда не бывает ровно 1.0 - сжатие
            // оставляет 0.99, и полностью непрозрачные детали начинают слегка
            // просвечивать. Порог поднимает почти-непрозрачное до единицы,
            // контраст углубляет полутона, не трогая непрозрачные участки.
            ...
            half4 OpacityFragment(Varyings IN, half facing : VFACE) : SV_Target
            {
                UNITY_SETUP_INSTANCE_ID(IN);
                ...
                half occlusion = LerpWhiteTo(orm.r, _OcclusionStrength);

                #ifdef _ORM_G_IS_SMOOTHNESS
                half smoothness = saturate(orm.g * _RoughnessScale);
                #else
                half smoothness = saturate(1.0h - orm.g * _RoughnessScale);
                #endif
                smoothness = max(smoothness, _SmoothnessMin);

                half metallic = saturate(orm.b * _MetallicScale);

                half3 vertexNormal = normalize(IN.normalWS) * (facing > 0 ? 1.0h : -1.0h);
                ...
                half alpha = RemapOpacity(baseSample.a);
                ...
                // premultiplied: альбедо гасим альфой, отражения оставляем
